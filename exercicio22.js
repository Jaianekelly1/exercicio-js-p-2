...
function numeroDaSorte(numeroEscolhido) {
    const min = 1
    const max = 10
    const numerosAleatorios = Math.floor(Math.random() * (max - min + 1) + min)

    if(numeroEscolhido === numerosAleatorios) {
        return `Parabéns! O número sorteado foi o ${numerosAleatorios}`
    } else {
        return `Que pena! O número sorteado foi o ${numerosAleatorios}`
    }
}
